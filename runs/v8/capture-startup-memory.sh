...
OUT=/home/tj/.cache/ds41rt-v8-package/startup-memory.json
...
started="$(docker inspect ds41rt-v8-published --format '{{.State.StartedAt}}' 2>/dev/null)"
[[ -n "$started" ]] || { echo "no running ds41rt-v8-published container" >&2; exit 1; }

ready="$(docker logs ds41rt-v8-published 2>&1 | sed 's/\x1b\[[0-9;]*m//g' \
