...
impl StartupPlacement {
    /// The launcher supplies a fresh private directory shared with the container.
    /// Publishing is atomic and refuses a previous launch's plan or ready marker.
    pub fn publish(directory: &Path, layers: usize) -> Result<Self> {
            ...
            directory.is_dir(),
            "placement handoff directory does not exist"
        );
        ensure!(
    ...
    fn acknowledged(&self) -> Result<bool> {
        let path = self.directory.join("ready.json");
        let mut file = match fs::File::open(&path) {
            Ok(file) => file,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(false),
            Err(e) => return Err(e).context("opening worker placement acknowledgement"),
        };
        use std::io::Read;
        ...
        (&mut file).take(4097).read_to_end(&mut bytes)?;
            ...
            bytes.len() <= 4096,
            "worker placement acknowledgement exceeds size limit"
        );
        let ready: Plan =
            serde_json::from_slice(&bytes).context("reading worker placement acknowledgement")?;
        ensure!(
            ready == self.plan,
...
mod tests {
    ...
    fn placement_publication_and_exact_acknowledgement() -> Result<()> {
        for layers in [1, 17, 20, 40] {
            ...
            assert!(StartupPlacement::publish(directory.path(), layers).is_err());
        }
        Ok(())
    }
    #[test]
