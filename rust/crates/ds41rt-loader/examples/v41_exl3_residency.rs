...
use ds41rt_loader::{read_official_v41_catalog, V41Exl3Layer, OFFICIAL_V41_MODEL_ID};
...
fn main() -> Result<()> {
    ...
    anyhow::ensure!(
        args.len() >= 5,
        "usage: v41_exl3_residency SNAPSHOT LAYER|mtp:STAGE WORLD RANK [--read]"
    );
    ...
    let plan = catalog
        ...
        .residency(layer, args[3].parse()?, args[4].parse()?)?;
    let mut hashes = Vec::new();
    if args.get(5).is_some_and(|s| s == "--read") {
