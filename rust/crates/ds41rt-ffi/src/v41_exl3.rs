...
impl V41Exl3Kernel {
    ...
    pub unsafe fn load(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let query = *library.get::<unsafe extern "C" fn(*mut u32, u32) -> i32>(b"ds41rt_exl3_info")?;
        let create = *library.get::<unsafe extern "C" fn(*mut *mut c_void) -> i32>(b"ds41rt_exl3_create")?;
        ...
        let mut words = [0; 15];
        ensure!(query(words.as_mut_ptr(), 15) == 0, "EXL3 native info query failed");
        ensure!(words[0] == 1 && words[1] == 5120 && matches!(words[2], 512 | 640 | 1152 | 2304)
            && words[3] <= 384 && words[3] >= words[5] && words[4] > 0
            && matches!(words[5], 3 | 6) && matches!(words[6], 2 | 3), "invalid EXL3 native geometry: {words:?}");
        let tier_count = words[6] as usize;
        let bits: [u32; 4] = words[11..15].try_into().unwrap();
        ensure!(bits[..tier_count].iter().all(|bit| (2..=5).contains(bit))
            && bits[tier_count..].iter().all(|bit| *bit == 0), "invalid EXL3 native tier bits");
        ensure!(words[7..11].iter().all(|count| *count > 0 && *count <= 64), "invalid EXL3 pointer/scalar ABI");
        let info = V41Exl3Info { hidden: words[1] as usize, intermediate: words[2] as usize,
            experts: words[3] as usize, capacity: words[4] as usize, topk: words[5] as usize,
            tier_count, bits, core_pointers: words[7] as usize, core_scalars: words[8] as usize,
            sum_pointers: words[9] as usize, sum_scalars: words[10] as usize };
        let mut context = std::ptr::null_mut();
        let status = create(&mut context);
        ensure!(status == 0, "EXL3 native initialization failed with CUDA status {status}");
        let context = NonNull::new(context).context("EXL3 initialized a null context")?;
        Ok(Self { _library: library, context, info, core, sum, destroy, _owner_thread: PhantomData })
    }

    pub fn info(&self) -> V41Exl3Info { self.info }
    ...
    pub unsafe fn launch_core(&self, pointers: &[*mut c_void], scalars: &[i32], stream: *mut c_void) -> Result<()> {
        ensure!(pointers.len() == self.info.core_pointers && scalars.len() == self.info.core_scalars,
            "EXL3 core argument count mismatch");
        let status = (self.core)(self.context.as_ptr(), pointers.as_ptr(), scalars.as_ptr(), stream);
    ...
    pub unsafe fn launch_sum(&self, pointers: &[*mut c_void], scalars: &[i32], stream: *mut c_void) -> Result<()> {
        ensure!(pointers.len() == self.info.sum_pointers && scalars.len() == self.info.sum_scalars,
            "EXL3 sum argument count mismatch");
        let status = (self.sum)(self.context.as_ptr(), pointers.as_ptr(), scalars.as_ptr(), stream);
