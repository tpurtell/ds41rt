...
- [x] Native selftest passes on the final CUDA source for SM120; SM121 compiles.
- [x] The 1024-thread GPU sampler's seeded CPU/GPU residual rates are recorded
  per cell from the final binary, with exact mask, tie, greedy, and boundary
  cases passing.
- [x] The normal and constrained live API qualifier passes on the candidate
  image pair, including seeded replay.
- [x] Fixed-logit latency is measured on all five requested profiles at one,
...
- [x] `./build.sh --config ds41rt.build-v12.config --dry-run` selects the v12
  coordinator and Spark pair; the runtime default selected v11 until the
  publication gate was complete.
- [x] Build exits successfully, artifact verification passes, and both images
...
- [x] Five discarded warmups and three interleaved repeats of the five canonical
  modes complete on nine content types plus the weight-zero counting diagnostic.
- [x] `validate-sampling-campaign.py` passes on 15 raw reports and the captured
...
- [x] README and [release-v12-performance.md](release-v12-performance.md) name
...
- [x] Capture the v11 `latest` registry digests before pushing and confirm both
  `:v12` tags are absent.
- [x] Publish exactly the v12 coordinator and Spark images and their `latest`
  aliases; capture the registry's returned digests.
- [x] Verify fresh anonymous pulls on the matching architectures and archive
  publication evidence with `SHA256SUMS`.
- [x] Promote `ds41rt.config` and examples to v12, verify dry runs, update
...
- [x] Publish a non-draft GitHub release with the evidence tarball, its payload
  `SHA256SUMS`, and the asset-level SHA-256 manifest.
